	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			$(document).on("click", "#selectAllBtn", function(){
				location.href="board_question_selectAll.p";
			});

			$(document).on("click", "#updateBtn", function(){
				alert("updateBtn");
				$("#boardUpdateForm").attr({ "method":"GET", "action":"board_question_select.p"}).submit();
			});
...
<form name="boardUpdateForm" id="boardUpdateForm">
	<div class="post-detail">
		<input type="hidden" id="boardquestionid" name="boardquestionid" class="boardquestionid" value=<%= _bqvo.getBoardquestionid() %>>
		<h2><%= _bqvo.getTitle() %></h2>
		<p>작성자: <%= _bqvo.getUserid() %></p>
		<p>작성일: <%= _bqvo.getCreateddate() %></p>
		<p>조회수: <%= _bqvo.getViewcount() %></p>
		<div class="post-content">
			<img src="/dogFoot/fileupload/board/<%= _bqvo.getPicturefile() %>" width="200" height="200" alt="image">
			<%= _bqvo.getContent() %>
		</div>
...
<!-- 댓글 처리 하는 루틴  -->
<jsp:include page="comment_question_form.jsp">
	<jsp:param value="<%=_bqvo.getBoardquestionid()%>" name="boardquestionid"/>
</jsp:include>
